...
# : 파이썬의 예약어, 첫 문자를 항상 대문자로 사용해야 한다.
...
# 출력값 : <class 'int'> 변수 a의 자료형은 int 이다.
...
# 큰 따옴표(""), 작은 따옴표(''), 큰 따옴표 3개 연속 표시(""""""),  작은 따옴표 3개 연속 표시('''''')
...
message = "I Like you!"
# len() : 문자열의 길이를 구하는 파이썬의 내장 함수 (length)
# 문자열의 경우 공백, 기호를 포함
...
# 1) %-포매팅(교재 p.56 - %연산자)
# %d: 숫자 포맷 코드
# %s: 문자열 포맷 코드(어떤 형태의 값이든 변환해서 삽입가능)
print('Hello, %s' %'python') #Hello, python
print('I have %d apples.' %4) #I have 4 apples.
...
# f-string 포매팅의 장점:  표현식 지원( 문자열 안에서의 변수와, +, - 의 수식을 함께 사용 가능)
a = 5;
b = 3;
print(f'Five times three is {a * b}'); # Five times three is 15
...
print(str.find('r')) #4
print(str.find('k')) #-1 : 문자열안에 해당 문자가 없는 경우 -1 출력
