...
# 3. else 문
# if문의 조건이 충족되지 않을 때(즉, if문으이 조건이 False일 때) 실행되는 코드 블록을 지정
'''
if 조건:
    실행할 코드
else:
...
if num > 10:
    ...
    print('반갑습니다') # 10이하의 수를 조건으로 하는 else문
...
# 돈이 3000보다 많거나 카드가 있을 경우 '택시를 탑니다'를 출력
# 그렇지 않을 경우 '걸어갑니다'출력
if money > 3000 or card: # card는 이미 있기 때문에 #False or True: True0
    print('택시를 탑니다.')
else:
    print('걸어갑니다')
...
if num > 0:
    print('양수입니다') #양수입니다는 num이 10보다 크면 무조건 실행
    if num > 10:
...
if 조건1:
    ...
    실행할 코드2(조건 1이 거짓리고 조건2가 참일 때 실행할 코드)
...
elif 조건 999:
    실행할 코드999
else:
...
#어떤 숫자가 10보댜 큰지 아닌지를 판단하여 문자열을 반환하는 삼항 연산자
num = 15
