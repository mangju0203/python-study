...
# 1. chr() 함수
# : 문자 자신만이 가지는 코드값(문자 코드 - 유니코드)를 반환
# : 무자 코드에는 미국 표중 코드(아스키 코드), 국제 표준 코드(유니 코드) 등이 있다
...
# 4. format()함수
# : 전달받은 인수와 포맷 코드에 따라서 형식를 갖춘 문자열 반환
print(format(10000)) #숫자형 10000이 문자형 '10000'으로 출력
print(format(10000, ',')) # 10,000 숫자를 천 단위로 표현할 때 사용
