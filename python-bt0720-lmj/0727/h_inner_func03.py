...
# 1. enumerate() 함수 - 리스트
# 리스트에 저장된 요소와 해당 요소의 인덱스가 튜플(tuple)형태로 함께 추출 (출력값이 튜플 값인 소괄호에 담겨져 나옴)
...
# 3. len() 함수
# : 컨테이너에 있는 원소의 개수를 반환 (컨테이너는 모든 괄호)
# : 리스트, 튜플, 문자열, 딕셔너리
...
my_string = 'Hello, Python' #문자열은 공백, 기호 포함
...
my_list = [5, 1, 6, 3, 7]
# 오름차순 정렬
print(sorted(my_list)) # [1, 3, 5, 6, 7]

# 내림차순 정렬
print(sorted(my_list, reverse=True)) # [7, 6, 5, 3, 1]
